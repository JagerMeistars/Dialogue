# Генератор диалогов и мыслей dcs (схема спеццветов — в шапке text.vsh).
...
# мысли: список вариантов, вариант = список окон (ru, en)
thoughts = {
    "jagermeistars": [
        [(("Спать хочу... Сон полезен для здоровья,", "а здоровье нужно всем..."),
          ("I wanna sleep... Sleep is good for you,", "and everyone needs their health..."))],
        [("Это отдам Клоду, это сделаю сам...",
          "This one goes to Claude, this one I'll do myself...")],
        [("Да здесь всё перепутано! Как оно работает вообще?!",
          "Everything's tangled up in here! How does it even work?!")]],
    "chayosi": [
        [("«//ДОСТУП К МЫСЛИ ЗАБЛОКИРОВАН//»",
          "'//THOUGHT ACCESS DENIED//'", "glitch")],
        [("Осталось совсем чуть-чуть...",
          "Just a little bit left...")],
        [("«//Мысль C137 успешно прибыла в нейрон BC-59601984//»",
          "'//Thought C137 delivered to neuron BC-59601984//'", "glitch")]],
    "dreamliner": [
        [("Когда, когда карта... Да завтра. Все всегда завтра.",
          "When, when is the map... Tomorrow. Always tomorrow.")],
        [("Ну емае, Моджанг: почему что-то добавляют — но не до конца?",
          "C'mon Mojang, why add stuff — and never finish it?")],
        [("Это какой-то полный пу-пу-пу, а не датапак.",
          "This is a total poo-poo-poo of a datapack.")],
        [("А что если?.. Да не, фигня какая-то.",
          "What if?.. Nah, that's dumb.")]],
    "smilekek": [[
        (("Боже, почему меня заставляют отыгрывать", "роль, что не свойственна мне..."),
         ("God, why do they make me play a role", "that was never mine...")),
        (("Это ужасно, я правда не псих, но создатели дали", "мне лишь оболочку, голосящую одни и те же слова..."),
         ("It's awful. I'm not actually crazy, but the creators", "gave me only a shell that yells the same words...")),
        ("Когда я выйду из этого порочного круга?",
         "When do I get out of this vicious circle?")]],
    "dafigvam": [
        ...
        [("Нет слов...",
          "No words...")],
        [("Щас бы бамбук...",
          "Man, I could use some bamboo...")],
        [("Это нужно репостнуть в Vanilla Maps.",
          "This needs a repost to Vanilla Maps.")],
        [("Сегодня поработаем, а завтра на дачу.",
          "Work today, dacha tomorrow.")],
        [("🍄",
          "🍄")]],
}
...
def seg_component(key, fx, step, thought, row2):
    wave = 8 if thought else 0
    it = ',"italic":true' if thought else ""
    if fx:
        g = FX_IDS[fx] | (8 if row2 else 0) | (step << 4)
        return f'{{"translate":"{key}","color":"#FA{g:02X}00","shadow_color":0}}'
    if row2:
        return f'{{"translate":"{key}","color":"#F9{wave | (step << 4):02X}00","shadow_color":0{it}}}'
    if thought:
        return f'{{"translate":"{key}","color":"#FB{wave | (step << 4):02X}00","shadow_color":0{it}}}'
    return f'{{"translate":"{key}","color":"{dim((255,255,255), step)}","shadow_color":0{it}}}'

def window_json(nid, name, seglists, step, thought, two_row):
    """seglists: [(key, fx, row2), ...] в порядке компонентов"""
    icon = (THOUGHT_ICON_CHARS if thought else ICON_CHARS)[nid]
    ...
    for key, fx, row2 in seglists:
        parts.append(seg_component(key, fx, step, thought, row2))
    return "[" + ",".join(parts) + "]"
...
def emit_seq(key, nid, name, windows, thought, sound_lines):
    folder = "thought" if thought else "dialog"
    wjs = []   # список seglists на окно
    durs = []
    for i, win in enumerate(windows, 1):
        ru, en = win[0], win[1]
        fx_all = win[2] if len(win) > 2 else None
        base = f"dcs.{'t' if thought else 'd'}.{key}.{i}"
    ...
    dur = sum(durs) + 10
    w = lambda i, step=7: window_json(nid, name, wjs[i], step, thought, any(r2 for _, _, r2 in wjs[i]))
    tagname = f"thought_{key}" if thought else key
    ...
    for i in range(len(windows)):
        ...
        if i > 0:
            if thought:
                steps.append(f"execute if score @s dcs.dlg matches {hi} run playsound minecraft:block.amethyst_block.resonate player @s ~ ~ ~ 1 {round(0.7 * (1.12 if i % 2 else 0.94), 2)}")
            else:
...
for key, (nid, name, voice, pitch, reps) in npcs.items():
    ...
    if isinstance(reps, dict):
        ...
        for v, rl in reps.items():
            ROUTING.append(emit_seq(f"{key}_{v}", nid, name, rl, False, snd))
            vkeys.append(f"{key}_{v}")
        write_dispatcher(key, "dialog", vkeys)
        print("dialog ok", key, f"({len(reps)} набора)")
    else:
        ROUTING.append(emit_seq(key, nid, name, reps, False, snd))
        print("dialog ok", key)

for key, variants in thoughts.items():
    nid, name, voice, pitch, _ = npcs[key]
    VOICE, PITCH, VOL = voice, pitch, 1.0
    snd = ["playsound minecraft:block.amethyst_block.resonate player @s ~ ~ ~ 1 0.7"]
    if len(variants) > 1:
        vkeys = []
        for i, wv in enumerate(variants, 1):
            ROUTING.append(emit_seq(f"{key}_{i}", nid, name, wv, True, snd))
            vkeys.append(f"{key}_{i}")
        write_dispatcher(key, "thought", vkeys)
        print("thought ok", key, f"({len(variants)} варианта)")
    else:
        ROUTING.append(emit_seq(key, nid, name, variants[0], True, snd))
        print("thought ok", key)
...
for key, wins in hits.items():
    ...
    VOICE, PITCH, VOL = voice, pitch, (1.0 if "candle" in voice else 0.9)
    snd = [f"playsound minecraft:{voice} player @s ~ ~ ~ {VOL} {pitch}"]
    ROUTING.append(emit_seq(f"{key}_hit", nid, name, wins, False, snd))
    print("hit ok", key)
